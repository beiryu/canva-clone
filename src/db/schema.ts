...
/**
 * User-authored style presets, generated by reading a reference image with a
 * vision model. Scoped to the user rather than a project — a look you like
 * applies across every design.
 */
export const stylePresets = pgTable("style_preset", {
  id: text("id")
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
  userId: text("userId")
    ...
    .references(() => users.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  /** Injected verbatim as the [STYLE GUIDANCE] block at generation time. */
  instruction: text("instruction").notNull(),
  /** Supabase fullPath of the reference image; doubles as the preview tile. */
  referencePath: text("referencePath"),
  /** Which vision model authored the instruction. */
  model: text("model"),
  createdAt: timestamp("createdAt", { mode: "date" }).notNull(),
  updatedAt: timestamp("updatedAt", { mode: "date" }).notNull(),
});
...
export const stylePresetsInsertSchema = createInsertSchema(stylePresets);
